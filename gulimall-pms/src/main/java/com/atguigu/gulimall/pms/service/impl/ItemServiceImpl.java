...
public class ItemServiceImpl implements ItemService {
    ...
    public SkuItemDetailVo getDetail(Long skuId) throws ExecutionException, InterruptedException {
        ...
        //1、当前sku的基本信息   2s
        //将一个异步任务交由线程池运行  （我们自己创建的一个线程池）
        //CompletableFuture.runAsync(()->{},mainThreadPool);  run：无返回值
        // CompletableFuture.supplyAsync(()->{return 1;})  ;    supply：有返回值  在默认线程池里执行，要指定用我们的线程池mainThreadPool
...
//        future.get()
        ...
        future.get();  //在线等   相当于阻塞代码，前面几个都执行完了，这个阻塞代码才可以通过，但是get（）是没东西的，因为他是一个总的
        //多线程；提升吞吐量；
        return detailVo;
    }
}
